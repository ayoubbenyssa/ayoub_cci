...
class FeederationsNews extends StatefulWidget {
  FeederationsNews(this.user, this.chng, this.fed_id, this.title);
...
class _FeederationsNewsState extends State<FeederationsNews> {
  ...
  getLIst() async {
    ...
    var a = await PostsServices.get_pub_type_commissions(
        skip * 4, "federations", widget.user, verify, widget.fed_id);
  ...
  showwidgets(List<Offers> result) {
    ...
    for (Offers item in result) {
      ...
      listWidget
          .add(Wall_card(item, widget.user, [], 0.0, 0.0, null, widget.chng));
    }
