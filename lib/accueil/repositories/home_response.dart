...
class HomeRepository {
  ...
  Future<HomeResponse> get_posts_by_type(String type, int skip, String class_db,
    ...
    if (type == "")
      query = QueryBuilder<ParseObject>(ParseObject(class_db))
        ...
        ..includeObject(["author", "partner","membre"])
      ...
      query = QueryBuilder<ParseObject>(ParseObject(class_db))
        ..whereEqualTo("type", type)
        ..includeObject(["author", "partner", "options", "options.users", "membre"])
        ..setLimit(15)
        ..orderByDescending("createdAt")
        ..setAmountToSkip(skip);
    }
  ...
  static Future<HomeResponse> get_events() async {
    String url = 'offers?where={"type":"event"}&order=-createdAt&include=partner,membre';
  ...
  static Future<HomeResponse> get_posts() async {
    String url = 'offers?where={"type":"news"}&order=-createdAt&include=membre';
    var getcity = await parse_s.getparse(url);
    if (getcity == "No")
      return HomeResponse(responseCode: 0, message: null, publications: []);
    if (getcity == "error")
  ...
  static Future<HomeResponse> get_allVideos(all) async {
    String url = 'offers?where={"type":"videotheque"}&order=-createdAt&include=include=membre,author,partner';
    var getcity = await parse_s.getparse(url);
    if (getcity == "No")
      return HomeResponse(responseCode: 0, message: null, publications: []);
    if (getcity == "error")
