...
class RegionsList extends StatefulWidget {
  RegionsList(this.lat, this.lng, this.id);
...
class _EntreproseListState extends State<RegionsList> {
  ...
  bool _IsSearching;
  String _searchText = "";
  List<Region> ent = new List<Region>();

  getList() async {
    var a = await PartnersList.get_list_regions();
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: load?[Center(child: Widgets.load())]:ent
            .map((val) => new Region_card(val, func, _searchText, sete))
            .toList(),
      ) /*StreamParcPub(
